                    ...
                    image.map((image, index) => {
                        return <div className='relative flex justify-center items-center group' key={index}>
                            ...
                            <div className={`bg-white bg-opacity-0 absolute ${mode === 'full' ? 'w-full h-full rounded-lg' : mode === 'dp' ? 'w-60 h-60 rounded-full' : mode === 'banner' ? 'w-full h-64 rounded-lg' : ''} text-2xl flex gap-4 items-center justify-center group-hover:bg-opacity-50 transition-all duration-300 ease-in-out`}>
                                <BiCamera onClick={() => onImageUpdate(index)} className='cursor-pointer opacity-0 group-hover:opacity-100 bg-darkModeBG text-white p-2 rounded-full text-4xl' />
                                <AiOutlineClose onClick={() => {
                                    setIsImageChange(pre => false)
                                    onImageRemove(index)
                                }} className='cursor-pointer opacity-0 group-hover:opacity-100 bg-darkModeBG text-white p-2 rounded-full text-4xl' />
                            </div>
                        </div>
                    }) : <section onClick={onImageUpload} {...dragProps} className='border-dashed border-2 border-darkModeBG p-8 flex justify-center items-center w-full h-60'>
